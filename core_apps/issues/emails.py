...
def send_issue_confirmation_email(issue: Issue) -> None:
    try:
        ...
        logger.error(
            f"Error to send confirmation email for issue '{issue.title}': {e}",
            exc_info=True
        )
...
def send_issue_resolved_email(issue: Issue) -> None:
    try:
        ...
        logger.error(
            f"Error to send resolution email for issue '{issue.title}': {e}",
            exc_info=True
        )
...
# os dados aqui estão em CASCATA, vem do models.py para template que acessamos aqui
